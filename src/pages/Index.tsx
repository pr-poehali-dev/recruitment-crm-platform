...
function CommPanel({ lead, onClose, onNewComm }: { lead: Lead; onClose: () => void; onNewComm: (c: Communication) => void }) {
  ...
  useEffect(() => {
    fetch(`${LEADS_URL}?action=communications&lead_id=${lead.id}`)
      .then(r => r.json())
      .then(d => { setComms(d); setLoading(false); });
  }, [lead.id]);

  async function addComm(e: React.FormEvent) {
    e.preventDefault();
    if (!text.trim()) return;
    setSaving(true);
    const res = await fetch(LEADS_URL, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ action: "add_communication", lead_id: lead.id, comm_date: date, text }),
    });
  ...
  return (
    <div className="fixed inset-0 z-50 flex">
      ...
      <div className="relative ml-auto w-full max-w-md bg-background flex flex-col h-full shadow-xl">
        ...
        <div className="flex items-start justify-between px-5 py-4 border-b border-border shrink-0">
          <div>
            ...
            <div className="text-xs text-muted-foreground mt-0.5 flex items-center gap-2">
              ...
              {lead.manager_display || lead.manager_name}
            </div>
          </div>
        ...
        {/* Форма добавления */}
        <form onSubmit={addComm} className="px-5 py-4 border-b border-border shrink-0 space-y-3">
          <div className="text-xs font-medium text-muted-foreground uppercase tracking-wide mb-2">Новая запись о коммуникации</div>
          ...
          <button
            ...
            className="w-full bg-primary text-primary-foreground text-sm py-2 rounded-lg hover:bg-primary/90 transition-colors disabled:opacity-50"
          >
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          <div className="text-xs font-medium text-muted-foreground uppercase tracking-wide mb-3">История ({comms.length})</div>
          {loading ? (
            <div className="flex items-center justify-center py-8 text-muted-foreground">
              <Icon name="Loader" size={18} className="animate-spin mr-2" />Загрузка…
            </div>
          ) : comms.length === 0 ? (
            <div className="text-center py-10 text-sm text-muted-foreground">Коммуникаций ещё нет</div>
          ) : (
            <div className="space-y-3">
              {comms.map(c => (
                <div key={c.id} className="bg-muted/30 rounded-xl px-4 py-3 border border-border/60">
                  <div className="text-xs text-muted-foreground mb-1 font-mono-nums">
                    {new Date(c.comm_date).toLocaleDateString("ru-RU", { day: "numeric", month: "long", year: "numeric" })}
                  </div>
